...
export async function render(el, deps = {}) {
  ...
  imgC.style.cssText = `
    ...
    will-change: opacity, transform, filter;
    /* メインロゴが出てから少し遅れてキラーン */
    animation: cFlash .9s ease-in-out 1.1s forwards;
    pointer-events:none;
  `;
  ...
  if (!document.getElementById("title-anim-css")) {
    ...
    st.textContent = `
      ...
      @keyframes cFlash {
        ...
        30%  { opacity:1; transform:scale(1.12); filter:brightness(1.9); }
        65%  { opacity:1; transform:scale(1.0);  filter:brightness(1.0); }
